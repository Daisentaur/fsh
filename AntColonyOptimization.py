...
class Graph(object):
    def __init__(self,rank: int,cost_matrix: list,speed=float):
        """
        :param cost_matrix:
    ...
    def calculate_time_matrix(self):
        return [[distance / self.speed for distance in row] for row in self.cost_matrix]

    def get_distance(self, from_node: int, to_node: int) -> float:
        return self.cost_matrix[from_node][to_node]

    def get_time(self, from_node: int, to_node: int) -> float:
...
class AntColony(object):
    ...
    def solve(self, graph: Graph):
        ...
        total_distance = sum(graph.cost_matrix[path[i]][path[i + 1]] for i in range(len(path) - 1))
        total_time = time_between_ports(total_distance)
        return path, cost, total_time
    ...
    def _update_pheromone_level_delta(self):
        ...
        for _ in range(1, len(self.tabu)):
            ...
            if self.colony.update_strategy == 1:  # ant-quality system
                ...
                self.pheromone_level_delta[i][j] = self
